// src/app/modules/appointments/appointment.model.ts
...
// Define AppointmentDocument as an intersection of Mongoose Document and IAppointment
type AppointmentDocument = Document & IAppointment;
...
appointmentSchema.statics.getAvailableSlots = async function (
  ...
  // Generate time slots based on service duration (in minutes)
  for (
...
appointmentSchema.pre<AppointmentDocument>('save', async function (next) {
  // Set cancellation deadline (24 hours before the appointment)
  ...
  // Business hours validation when appointmentDate or startTime changes
  if (
    this.isNew ||
    ...
    console.log('Salon in appointment model::', salon);
    ...
    console.log('Salon open time::', businessHours);
    if (!businessHours) {
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
